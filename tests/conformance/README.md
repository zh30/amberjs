...
`express_smoke.js`, `fastify_smoke.js`, and `hono_smoke.js` load Express, Fastify, and Hono from `benchmarks/idle_memory/node_modules` (gitignored). If those directories are missing they print `CONFORMANCE_SKIP`. CI installs `express@5.2.1`, `fastify@5.12.3`, `hono@4.13.5`, and `@hono/node-server@2.1.1` before the scorecard. Locally:
