---
title: "Wasm 2.0 (bee:wasm)"
subtitle: "Preview in v1.16.0: zero-copy WebAssembly.Memory and ArrayBuffer"
group: "Agent & Advanced"
id: "wasm-interop"
---

v1.16.0 adds a **Preview** `bee:wasm` module so JavaScript and WebAssembly can share the same backing store instead of copying buffers at the boundary.

This is not a new Wasm engine. V8 still compiles and runs Wasm. Beejs adds host helpers around `WebAssembly.Memory` / `ArrayBuffer`.
...
- `require('bee:wasm')` / `import wasm from 'bee:wasm'`
...
FFI pointer wrapping and `bee:ai.Tensor` views exist on the same module; treat those helpers as Preview too.
