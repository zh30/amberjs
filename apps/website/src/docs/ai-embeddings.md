---
title: "Native Text Embeddings (amber:ai 2.0)"
...
Amber v1.3.0 introduces a **zero-dependency, native text vectorization engine**:
...
Import embedding utilities from the `amber:ai` module:
...
| `asTensor` | `boolean` | `false` | Return a `amber:ai` `Tensor` instance instead of `Float32Array` |
...
const docs = [
  { id: '1', text: 'Amber v1.3.0 ships official Model Context Protocol 2.0.' },
...
const query = "How does Amber safely run untrusted Agent code?";
