...
在 Amber 中，每个 V8 Isolate 拥有专属的堆内存空间（Heap）。理解 V8 的分代式垃圾回收机制对于保障高并发服务的长周期稳定运行至关重要：
...
### 为什么 Amber 的 Buffer 速度位列第一？
在官方 100,000 次 64KB 缓冲区操作基准测试中，Amber 仅耗时 **2.09ms**，全面领先 Node.js (4.80ms) 与 Bun (2.50ms)：
