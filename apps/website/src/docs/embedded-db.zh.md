---
title: "嵌入式数据与向量引擎 (amber:db & amber:vector)"
...
Amber 在底层直接使用 Rust 打包编译了原生的 **SQLite 3** 引擎与轻量级**高维向量检索（VectorDB）引擎**，通过 `amber:db` 与 `amber:vector` 提供开箱即用、零外部依赖的高性能数据体验。

---

## 2. 嵌入式 SQLite (`amber:db` / `amber:sqlite`)
...
## 3. 向量检索数据库 (`amber:vector`)

`amber:vector` 为本地 RAG（检索增强生成）、知识库搜索与推荐算法量身定制。支持在本地内存中索引上万条高维嵌入向量，并提供微秒级的最近邻相似度排序。
...
结合 `amber:ai`、`amber:db` 与 `amber:vector`，你可以在 30 行内使用纯 TypeScript 构建一个完全脱离 Python 与外部容器的自包含 AI 问答检索服务：
