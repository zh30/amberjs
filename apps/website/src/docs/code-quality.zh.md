...
`amber fmt` 能够以比 Prettier 快数十倍的速度格式化 JavaScript、TypeScript、JSX 和 TSX 文件。
...
`amber lint` 基于 Rust AST 进行极速语法分析与危险模式拦截，无需繁杂的 `.eslintrc` 配置文件即可开箱即用。
...
当代码中存在语法错误、未捕获的死循环或危险 API 时，`amber lint` 会以美观终端高亮输出具体位置与修复建议：
...
| 工具 | 耗时 | 内存占用 | 依赖要求 |
| :--- | :--- | :--- | :--- |
| **`amber fmt` (OXC)** | **~3.8 ms** | **< 15 MB** | **零依赖** |
| Prettier v3.2 | ~420 ms | ~120 MB | Node.js + npm |
| **`amber lint` (OXC)** | **~4.5 ms** | **< 18 MB** | **零依赖** |
