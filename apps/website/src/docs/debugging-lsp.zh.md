...
2. **CDP 调试器 (`amber debug` / `--inspect`)**：兼容 Chrome 开发者工具与 VS Code 的远程调试服务；
3. **语言服务器 (`amber lsp`)**：符合 LSP 3.17 标准，为现代 IDE 提供亚毫秒级诊断、格式化与文档悬停。
...
- **历史记录持久化**：所有执行过的命令自动持久化保存至用户主目录 `~/.amber_history`，支持通过上下方向键自由翻看；
...
## 2. Chrome DevTools 协议调试 (`amber debug`)
...
- **`textDocument/formatting`**：无缝调用 `amber fmt` 提供零配置一键格式化文档；
