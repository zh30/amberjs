---
title: "企业级能力安全控制 (amber:security)"
...
**Amber v1.6.0 正式推出企业级能力安全引擎（`amber:security` 与 `amber:permissions`）**。完全对齐 W3C 权限查询标准，提供基于最小特权原则的沙箱策略收敛与动态权限撤销。
...
| 模块名称 | 接口签名 | 功能说明 |
| :--- | :--- | :--- |
| `amber:permissions` | `query(descriptor): PermissionStatus` | 探查指定资源描述符的权限状态（granted / denied） |
| `amber:permissions` | `has(descriptor): boolean` | 便捷返回权限是否被授予的布尔值 |
| `amber:permissions` | `list(): { allow, deny }` | 获取当前生效的全部底层白名单与黑名单规则 |
| `amber:permissions` | `revoke(descriptor): boolean` | 在运行时永久撤销某项能力 |
| `amber:security` | `createSandboxPolicy(rules): Policy` | 构建经过验证的标准沙箱权限策略对象 |
| `amber:security` | `attenuate(base, restriction): Policy` | 基于最小特权原则对两个策略执行交集衰减 |
