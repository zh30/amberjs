...
- **悬停信息 (Hover)**：显示内置 API（如 `amber:db`、`bee:vector`、`amber:std`）的 JSDoc 文档与函数签名；
...
| `Amber: Format Document` | 极速格式化当前文件 | 调用 `bee fmt <file>` |
| `Amber: Export TypeScript Types` | 导出最新内置类型定义 | 调用 `bee types` 生成 `beejs.d.ts` |
| `Amber: Deploy Application` | 交互式生成部署脚手架 | 调用 `bee deploy` |
