...
Amber 坚信：**开发者不应当为运行一行 TypeScript 代码而经历繁琐的编译配置。**
...
Amber 内置了由 Rust 编写的高性能 **oxc**（The Oxidation Compiler）编译器作为转译中枢：
...
除了基础的类型标注擦除，Amber 对现代 ECMAScript 提案与 TypeScript 6.0 提供了全面的前瞻支持：
...
### 3. TSX / JSX 原生降级
Amber 原生支持 JSX 语法，将其自动转换为标准的 `React.createElement` 调用：
...
与 Deno 和 Bun 一致，Amber 在运行脚本时采用 **Transpile-only** 策略：
- **目标是极致启动速度**：在生产环境和开发频繁调试时，类型检查（Type Checking）是非常耗时且冗余的操作。因此，Amber 仅在内存中擦除类型并降级语法，使启动时间控制在 18ms 以内；
