...
excerpt: "Zero-copy WebAssembly.Memory, amber bundle / compile (Preview), benchmark suite 2.0, and URL / fetch / ReadableStream RSI. Node Conformance 5.0 remains 55/55."
date: "2026-09-16"
...
v1.16.0 is a runtime release, not a marketing reset. The binary is still `amber`. The capability map is still [Current Scope](https://github.com/zh30/amberjs/blob/main/docs/CURRENT_SCOPE.md). Node.js Conformance 5.0 is still **55/55**.
...
- `amber compile` copies the host `amber` binary and appends a payload plus a `BEE_STANDALONE` trailer.
