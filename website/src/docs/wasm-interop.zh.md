...
v1.16.0 增加了 **Preview** 模块 `bee:wasm`：JavaScript 和 WebAssembly 可以共用同一块 backing store，不必在边界上拷贝 buffer。

这不是一个新的 Wasm 引擎。编译和执行仍由 V8 完成。Beejs 在 `WebAssembly.Memory` / `ArrayBuffer` 周围加了宿主辅助。
...
同一模块上还有 FFI 指针包装和 `bee:ai.Tensor` 视图，同样按 Preview 对待。
