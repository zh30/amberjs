// Multi-stage Dockerfile builder for Amber
...
impl MultiStageBuilder {
    ...
    fn generate_header(&self) -> Result<String, Error> {
        let header: _ = format!(
            r#"# Multi-stage Dockerfile for Amber
# Generated by Amber Cloud Native
# Builder: {} (Rust {})
# Runtime: {}
# Optimizations: {}
#
"#,
    ...
    fn generate_builder_stage(&self) -> Result<String, Error> {
        ...
        if self.builder_stage.strip_binaries {
            stage.push_str(" && strip target/release/amber");
        }
        if self.builder_stage.optimize_for_size {
            stage.push_str(" && objcopy --only-keep-debug amber amber.debug && strip amber && objcopy --add-gnu-debuglink=amber.debug amber");
        }
        stage.push_str("\n\n");
        Ok(stage)
    }
    ...
    fn generate_runtime_stage(&self) -> Result<String, Error> {
        ...
        stage.push_str("COPY --from=builder /app/target/release/amber /usr/local/bin/");
        // Set permissions
        if self.builder_stage.strip_binaries {
            stage.push_str(" && chmod +x /usr/local/bin/amber");
        }
        stage.push_str("\n");
        // Set ownership
        if self.runtime_stage.create_user {
            stage.push_str(&format!(
                "RUN chown {}:{} /usr/local/bin/amber\n",
        ...
        // Set entrypoint
        stage.push_str("ENTRYPOINT [\"amber\"]\n");
        Ok(stage)
    }
}
/// Builder stage configuration
...
impl Default for RuntimeStage {
    fn default() -> Self {
        let mut labels = HashMap::new();
        labels.insert("maintainer".to_string(), "Amber Team".to_string());
        labels.insert("version".to_string(), "1.0.0".to_string());
        Self {
            base_image: "debian:bookworm-slim".to_string(),
            create_user: true,
            username: "amberjs".to_string(),
            user_group: "amberjs".to_string(),
            labels,
            packages: Vec::new(),
        }
    }
