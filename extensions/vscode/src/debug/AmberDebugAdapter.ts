 ...
 * Amber Debug Adapter
 *
 * Launches `amber run --inspect-brk --inspect-port <port>` and speaks DAP
...
export class AmberDebugAdapterDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
    constructor(private readonly config: AmberConfiguration) {}

    public createDebugAdapterDescriptor(
        _session: vscode.DebugSession,
        _executable: vscode.DebugAdapterExecutable | undefined
    ): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
        return new vscode.DebugAdapterInlineImplementation(new AmberDebugAdapter(this.config));
    }
}

class AmberDebugAdapter implements vscode.DebugAdapter {
    ...
    constructor(private readonly config: AmberConfiguration) {
        this.outputChannel = vscode.window.createOutputChannel('Amber Debug');
    }
    ...
    public handleMessage(message: vscode.DebugProtocolMessage): void {
        ...
        switch (msg.command) {
            ...
            case 'variables':
            case 'continue':
                this.respond(msg, msg.command === 'threads' ? { threads: [{ id: 1, name: 'amber' }] } : {});
                break;
            default:
                this.respond(msg);
                break;
    ...
    private handleLaunch(request: DapRequest): void {
        ...
        const amberjsPath = this.config.getRuntimePath();
        const debugPort = this.config.getDebugPort();
        this.outputChannel.appendLine(
            `Launching: ${amberjsPath} run --inspect-brk --inspect-port ${debugPort} ${program}`
        );
        this.child = spawn(amberjsPath, [
            'run',
            '--inspect-brk',
            '--inspect-port',
            String(debugPort),
    ...
    private handleAttach(request: DapRequest): void {
        ...
        this.outputChannel.appendLine(`Attach to amber inspector on port ${port} (amber run --inspect)`);
        this.respond(request);
    }
